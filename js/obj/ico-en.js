...
//                      type  比特币0 以太坊1
                ...
                if(res.code=='200'){
                    ...
                    for(var i=0;i<data.length;i++){
                        ...
                        if(clickCoin==type){
                            ...
                            var code=address
                            showQrCode(code)
                            $('.jqcode-container .name').text(coin+' address')
    ...
    ico.prototype.exchange=function () {
        ...
        var that=this._that
        if(count==''){
            that.swal('The count is not empty',1000)
            return;
        }
        ...
        $.ajax({
            ...
            success: function(res){
                if(res.code=='200'){
                    that.swal('Successful trade！',1500)
                }else{
                    that.swal(word[res.msg],2000)
                }
            },
            error:function () {
               alert($.i18n.prop('network_error'));
    ...
    ico.prototype.applyForWhiteList=function(){
        ...
        $.ajax({
            ...
            success: function(res){
                if(res.code=='200'){
                    that.swal('request submitted',2000)
                }else{
                    that.swal(word[res.msg],2500)
                }
            },
            error:function () {
               alert($.i18n.prop('network_error'));
